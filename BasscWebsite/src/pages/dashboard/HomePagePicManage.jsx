...
const RESOURCE = 'homepagepic';

export default function HomePagePicManage() {
  const [list, setList] = useState([]);
  const [loading, setLoading] = useState(true);
  const [modalOpen, setModalOpen] = useState(false);
  const [editing, setEditing] = useState(null);
  const [form, setForm] = useState({ title: '', description: '', image: '', sort_order: 0, is_active: true });
  ...
  function openAdd() {
    setEditing(null);
    setForm({ title: '', description: '', image: '', sort_order: list.length, is_active: true });
    setImageFile(null);
    setSubmitError('');
    setModalOpen(true);
  }
            ...
            <tbody>
              {list.map((row) => (
                <tr key={row.id}>
                  ...
                  <td>{row.title}</td>
                  <td>{row.description?.slice(0, 50)}{row.description?.length > 50 ? '…' : ''}</td>
      ...
      {modalOpen && (
        <div className="dashboard-modal-overlay" onClick={() => setModalOpen(false)}>
          <div className="dashboard-modal" onClick={(e) => e.stopPropagation()}>
            <div className="dashboard-modal-header">{editing ? 'Edit Carousel' : 'Add Carousel'}</div>
            <form onSubmit={handleSubmit} className="dashboard-modal-body dashboard-form">
              ...
              <label>
                ...
                <div
                  ...
                  onClick={() => document.getElementById('pic-img').click()}
                >
